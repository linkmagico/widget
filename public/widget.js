...
(function(window, document) {
    ...
    // Security: Check for required token
    // O widget.js é carregado pelo widget-loader.js, que passa o token como 'token' na URL.
    // O widget-loader.js obtém o token como 'apiKey' em sua própria URL.
    const scripts = document.querySelectorAll('script[src*="widget.js"]');
    const currentScript = scripts[scripts.length - 1];
    const urlParams = new URLSearchParams(currentScript.src.split('?')[1] || '');
    
    // O token pode vir como 'token' (se carregado diretamente) ou 'apiKey' (se carregado pelo loader)
    let widgetToken = urlParams.get('token');
    if (!widgetToken) {
        widgetToken = urlParams.get('apiKey');
    }

    if (!widgetToken) {
        console.error('LinkMagico Widget: Token required (apiKey or token parameter missing)');
        return;
    }
